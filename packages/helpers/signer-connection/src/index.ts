...
export class SignerConnection extends IJsonRpcConnection {
  ...
  public async open(chainId?: string): Promise<void> {
    if (this.connected) {
      this.onOpen();
      return;
    }
  ...
  private create(chainId?: string): void {
    this.wc = this.register(this.opts);
  ...
  private registerConnectorEvents() {
    ...
    this.wc.on("connect", (err: IJsonRpcErrorMessage | null) => {
      if (err) {
    ...
    this.wc.on("disconnect", (err: IJsonRpcErrorMessage | null) => {
      if (err) {
        this.events.emit("error", err);
        return;
      }
  ...
  private async sendPayload(payload: any): Promise<JsonRpcResponse> {
    ...
    try {
      ...
      return this.onError(payload, (error as Error).message);
    }
  }
